...
os.chdir('/home/gdm/xfan/spaceOfWork/RE_torch')
...
# 加载数据集
train_data = load_data('./2020_baidu_RE/data/train_data.json')
valid_data = load_data('./2020_baidu_RE/data/dev_data.json')
'''3 稍微调整数据集，抛弃较长的句子'''
...
for data in tqdm(train_data):
    ...
    for s, p, o in data['spo_list']:
        ...
        if s_begin == -1 or o_begin == -1 or s_begin + len(
                s) > 250 or o_begin + len(o) > 250:
            flag = 0
            break
    if flag == 1:
...
'''4 读取schema'''
# 读取schema
with open('./2020_baidu_RE/data/schema.json', encoding='utf-8') as f:
...
class TorchDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        sub_labels = self.sequence_padding(sub_labels,
                                           maxlen=maxlen,
                                           padding=np.zeros(2))
        sub_ids = np.array(sub_ids)
        obj_labels = self.sequence_padding(obj_labels,
                                           maxlen=maxlen,
                                           padding=np.zeros(
                                               (len(predicate2id), 2)))

        return (torch.LongTensor(token_ids), torch.LongTensor(seg_ids),
                torch.LongTensor(sub_ids), torch.LongTensor(sub_labels),
    ...
    def sequence_padding(self, x, maxlen, padding=0):
        output = np.concatenate(
            [x, [padding] *
             (maxlen - len(x))]) if len(x) < maxlen else np.array(x[:maxlen])
        return output
...
class REModel(nn.Module):
    ...
    def forward(self, token_ids, seg_ids, sub_ids=None):
        ...
        sub_id1 = sub_ids[:, :1].unsqueeze(-1).repeat(
            1, 1, out.shape[-1])  # subject开始的位置id
        sub_id2 = sub_ids[:, 1:].unsqueeze(-1).repeat(
            1, 1, out.shape[-1])  # [batch_size, 1, size]
...
def extract_spoes(text, model, device):
    ...
    sub_preds = model(
        torch.LongTensor([token_ids]).to(device),
            ...
            for _start, predicate1 in zip(*start):
                for _end, predicate2 in zip(*end):
                    if _start <= _end and predicate1 == predicate2:
                        spoes.append(((sub[0] - 1, sub[1] - 1), predicate1,
                                      (_start - 1, _end - 1)))
                        break
...
def evaluate(data, model, device):
    ...
    f = open('./2020_baidu_RE/data/dev_pred.json', 'w', encoding='utf-8')
    pbar = tqdm()
    for d in data:
...
def train(model, train_loader, optimizer, epoches, device):
    for _ in range(epoches):
        ...
        for batch_idx, x in tqdm(enumerate(train_loader)):
            token_ids, seg_ids, sub_ids = x[0].to(device), x[1].to(
                device), x[2].to(device)
            # print(token_ids, seg_ids, sub_ids)
            mask = (token_ids > 0).float()
            mask = mask.to(device)  # zero-mask
            sub_labels, obj_labels = x[3].float().to(device), x[4].float().to(
                device)
            sub_preds, obj_preds = model(token_ids, seg_ids, sub_ids)
            # (batch_size, maxlen, 2),  (batch_size, maxlen, 55, 2)

            # 计算loss
            loss_sub = F.binary_cross_entropy(sub_preds,
                                              sub_labels,
                                              reduction='none')  # [bs, ml, 2]
            ...
            loss_obj = F.binary_cross_entropy(
                obj_preds, obj_labels, reduction='none')  # [bs, ml, 55, 2]
            ...
            print('loss: ', train_loss_sum / (batch_idx + 1), 'time: ',
                  time.time() - start)

        torch.save(net.state_dict(), "./2020_baidu_RE/data/bert_re.pth")
        with torch.no_grad():
            val_f1, pre, rec = evaluate(valid_data, net, DEVICE)
            print("f1, pre, rec: ", val_f1, pre, rec)


train(net, train_loader, optimizer, 20, DEVICE)
net.load_state_dict(torch.load("./2020_baidu_RE/data/bert_re.pth"))
'''10 推断'''


def combine_spoes(spoes):
    """ [('邪少兵王', '作者_@value', '冰火未央')]
    合并SPO成官方格式  [('邪少兵王', '作者', {'@value': '冰火未央'})]
    """
    new_spoes = {}
    for s, p, o in spoes:
        p1, p2 = p.split('_')
...
predict_to_file('./2020_baidu_RE/data/test1_data.json',
                f'RE_pred_{str(datetime.now())[:-10]}.json')
