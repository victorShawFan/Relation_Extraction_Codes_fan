...
from transformers import (AdamW, AlbertConfig, AlbertModel, AlbertTokenizer,
...
bert_path = 'albert-base-v2'  # 该文件夹下存放三个文件（'vocab.txt', 'pytorch_model.bin', 'config.json'）
tokenizer = AlbertTokenizer.from_pretrained(bert_path)  # 初始化分词器
'''# 2 预处理数据集'''
input_ids, input_masks, input_types, = [], [], [
]  # input char ids, segment type ids,  attention mask
labels = []  # 标签
maxlen = 30  # 取30即可覆盖99%
...
input_ids_train, input_ids_valid, input_ids_test = input_ids[
                                                       idxes[:80000]], input_ids[idxes[80000:90000]], input_ids[
                                                       idxes[90000:]]
input_masks_train, input_masks_valid, input_masks_test = input_masks[
                                                             idxes[:80000]], input_masks[idxes[80000:90000]], \
                                                         input_masks[idxes[90000:]]
input_types_train, input_types_valid, input_types_test = input_types[
                                                             idxes[:80000]], input_types[idxes[80000:90000]], \
                                                         input_types[idxes[90000:]]
...
class Bert_Model(nn.Module):
    def __init__(self, bert_path, classes=10):
        ...
        self.config = AlbertConfig.from_pretrained(bert_path)  # 导入模型超参数
        self.bert = AlbertModel.from_pretrained(bert_path)  # 加载预训练模型权重
